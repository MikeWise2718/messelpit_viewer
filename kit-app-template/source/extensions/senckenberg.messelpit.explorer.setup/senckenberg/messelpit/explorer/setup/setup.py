...
ICON_PATH = carb.tokens.get_tokens_interface().resolve("${senckenberg.messelpit.explorer.setup}/data/icons")
...
class SetupExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str):
        ...
        self._layouts_path = carb.tokens.get_tokens_interface().resolve("${senckenberg.messelpit.explorer.setup}/layouts")
        ...
        # Start in Layout mode rather than Review. Done after a few frame
        # yields so the menubar's ApplicationModeControl has already set its
        # default ("review") — flipping the carb setting then triggers the
        # mode-change subscription and loads the Layout panel layout.
        asyncio.ensure_future(self._switch_to_layout_mode_on_startup())
    ...
    async def _switch_to_layout_mode_on_startup(self):
        """Flip /app/application_mode to "layout" after the menubar has
        initialised. The ApplicationModeControl uses set_default("review") on
        startup, so we wait a few frames and then assign explicitly — that
        triggers _on_application_mode_changed and loads the default.json
        panel layout instead of the stripped comment_layout.json."""
        try:
            for _ in range(10):
                await self._app.next_update_async()
            current = self._settings.get_as_string(APPLICATION_MODE_PATH).lower()
            if current != "layout":
                self._settings.set_string(APPLICATION_MODE_PATH, "layout")
        except Exception as exc:  # pragma: no cover
            carb.log_warn(f"Failed to switch to Layout mode on startup: {exc}")
    ...
    def _custom_quicklayout_menu(self):
        ...
        def add_layout_menu_entry(name, parameter, key):
            ...
            layouts_path = carb.tokens.get_tokens_interface().resolve("${senckenberg.messelpit.explorer.setup}/layouts")
